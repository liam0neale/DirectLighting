...
namespace GData
{
	...
	static ID3D12Device* m_pDevice = nullptr; // direct3d device

	static IDXGISwapChain3* m_pSwapChain = nullptr; // swapchain used to switch between render targets

	static ID3D12CommandQueue* m_pCommandQueue = nullptr; // container for command lists

	static ID3D12DescriptorHeap* m_pRTVDescriptorHeap = nullptr; // a descriptor heap to hold resources like the render targets
	...
	static ID3D12GraphicsCommandList* m_pCommandList = nullptr; // a command list we can record commands into, then execute them to render the frame
