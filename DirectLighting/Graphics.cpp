...
bool Graphics::OnInit(LWindow &_window)
{
	...
	return Test(_window.getWidth(), _window.getHeight(), _window.getWindow());
}
...
void Graphics::CleanUp()
{
	...
	m_pDSDescriptorHeap->Release();

	for (int i = 0; i < m_frameBufferCount; ++i)
	{
		m_pConstantBufferUploadHeaps[i]->Release();
	};
}
...
bool Graphics::Test(int _width, int _height, HWND _hwnd)
{
	...
	// build projection and view matrix
	XMMATRIX tmpMat = XMMatrixPerspectiveFovLH(45.0f * (3.14f / 180.0f), (float)_width / (float)_height, 0.1f, 10000.0f);
	XMStoreFloat4x4(&m_cameraProjMat, tmpMat);
...
bool Graphics::Render22222()
{
	HRESULT hr;

	UpdatePipeLine22222(); // update the pipeline by sending commands to the commandqueue
...
bool Graphics::UpdatePipeLine22222()
{
	HRESULT hr;

	// We have to wait for the gpu to finish with the command allocator before we reset it
	WaitForPreviousFrame22222();
...
// create rotation matrices
 ...
 bool Graphics::InitD3D(int _width, int _height, HWND _hwnd)
{
	HRESULT hr;
...
void Graphics::WaitForPreviousFrame22222()
{
	HRESULT hr;
